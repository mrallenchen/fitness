...
def custom_score(y_test, y_pred_proba):
    score_points = np.array([[0,1,-1,0],[2.5,0,100,0],[0,.0,-100,.5]])
...
def PR_AUC_fitness(y_test,y_pred_proba):
    ...
    proba = np.concatenate(y_pred_proba).reshape(len(y_pred_proba),3)
    precision, recall, _ = precision_recall_curve(y_test_bin[:, 1], proba[:, 1])
    ...
    for indices in kf.split(X_train, y_train):
        ...
        if param_grid == None:
            ...
            for g in ParameterGrid(param_grid):
                ...
                train_class = classification_report(y_tr, model.predict(X_tr), target_names = ['Average','Fitness','Obese'], output_dict = True, zero_division=0)
                val_class = classification_report(y_val, model.predict(X_val), target_names = ['Average','Fitness','Obese'], output_dict = True,zero_division=0)
